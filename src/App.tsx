...
const SettingsIcon = () => <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6"><path strokeLinecap="round" strokeLinejoin="round" d="M10.343 3.94c.09-.542.56-1.007 1.11-.11.55.897.09 1.95-.56 2.597m0 0A7.5 7.5 0 0012 21a7.5 7.5 0 002.06-14.463m-4.12 0A7.5 7.5 0 0112 3.75a7.5 7.5 0 015.94 11.563m-5.94 0a7.5 7.5 0 00-5.94 11.563m11.88 0A7.5 7.5 0 0112 21a7.5 7.5 0 01-5.94-11.563m5.94 0a7.5 7.5 0 015.94-11.563M12 3.75a.75.75 0 01.75.75v.01c0 .414-.336.75-.75.75a.75.75 0 01-.75-.75v-.01c0-.414.336-.75.75-.75z" /></svg>;
...
const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    useEffect(() => {
        const processRecurringTransactions = () => {
            ...
            let updatedRecurringTxs = JSON.parse(JSON.stringify(recurringTransactions)) as RecurringTransaction[];

            updatedRecurringTxs.forEach((recTx) => {
                ...
                if (lastProcessed) {
                    ...
                    nextProcessingDate.setDate(Math.min(startDate.getDate(), new Date(nextProcessingDate.getFullYear(), nextProcessingDate.getMonth() + 1, 0).getDate()));
                }
    ...
    const projectionData = useMemo(() => {
        ...
        for (let i = 1; i <= 60; i++) {
            ...
            const monthlyRecurringNet = recurringTransactions.reduce((acc, recTx) => {
                ...
                const dayOfRecurrence = recStartDate.getDate();
                const recurrenceDateInMonth = new Date(futureDate.getFullYear(), futureDate.getMonth(), dayOfRecurrence);

                if (recurrenceDateInMonth.getMonth() !== futureDate.getMonth()) {
                    return acc;
                }
                
                if (recurrenceDateInMonth >= recStartDate && (!recEndDate || recurrenceDateInMonth <= recEndDate)) {
            ...
            <ResponsiveContainer width="100%" height={300}>
                <PieChart>
                    <Pie data={data} cx="50%" cy="50%" labelLine={false} outerRadius={100} fill="#8884d8" dataKey="value" nameKey="name"
                        ...
                        {data.map((_entry, index) => <Cell key={`cell-${index}`} fill={data[index].color} />)}
                    </Pie>
...
const PortfolioBreakdownChart: React.FC = () => {
    const { t, lang, investmentFinancials } = useAppContext();
    ...
    return (
        <Card>
            <h3 className="text-lg font-semibold text-on-surface mb-4">{t('portfolioBreakdown')}</h3>
            <ResponsiveContainer width="100%" height={250}>
                <PieChart>
                    ...
                    >
                        {portfolioBreakdown.map((_entry, index) => (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <Card>
             ...
             <Card>
                ...
                <div className="flex space-x-2">
                    <Button onClick={() => updateSettings({ theme: 'light' })} className={`w-full ${settings.theme === 'light' ? 'bg-primary text-white' : 'bg-on-surface/20 text-on-surface'}`}>{t('light')}</Button>
                    <Button onClick={() => updateSettings({ theme: 'dark' })} className={`w-full ${settings.theme === 'dark' ? 'bg-primary text-white' : 'bg-on-surface/20 text-on-surface'}`}>{t('dark')}</Button>
                </div>
            </Card>
        </div>
    );
